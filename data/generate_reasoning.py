...
def deepseek_qa(prompt: str, model: str):
    ...
    while answer is None and attempts < max_attempts:
        try:
            ...
            attempts += 1
            thinking, answer = answer["choices"][0]["message"]["content"].split(constants.DEEPSEEK_REASONING_TOKEN)                
...
def generate_deepseek(model):
    ...
    questions = [question for question in questions if utils.question_hash(question) not in existing_qhash_list]
